...
std::vector<Valtype> read_binary2vector(Valtype x, std::string filename) {
...
int main(int argc, char **argv) {
    ...
    std::ofstream file3(path + "/ppr/hwlist.bin", std::ios::binary | std::ios::app);
    ...
    float y=1.0;
    std::vector<ValueT> wlist = read_binary2vector(y,pwlist);
    ...
    //第一步：获取graph以及graphT
    std::vector<std::vector<std::pair<VertexT,ValueT>>> graph(vertex_cnt);
    csr_convert(graph, vlist, elist, wlist, vertex_cnt);
    wlist.resize(0);
    // 强制释放内存
    std::vector<ValueT>().swap(wlist);
    ...
    std::vector<std::pair<VertexT,ValueT>> tmp;
    std::vector<std::pair<VertexT,ValueT>> tmp1;
    std::vector<std::pair<VertexT,ValueT>> ans;
    std::vector<VertexT> root;
    std::vector<ValueT> dis;
            ...
            for(int k=0;k<j;k++){
                ...
                while(ii<graph[root[k]].size()||jj<tmp1.size()){
                    if (ii == graph[root[k]].size()) {
                        ans.push_back(
                                {tmp1[jj].first, tmp1[jj].second *0.8* dis[k]});
                        ++jj;
                        continue;
                    }
                    ...
                    if (std::get<0>(graph[root[k]][ii]) == tmp1[jj].first) {
                        ans.push_back({std::get<0>(graph[root[k]][ii]),std::get<1>(graph[root[k]][ii])+tmp1[jj].second *0.8* dis[k]});
                        ++ii;
                        ++jj;
                    } else if (std::get<0>(graph[root[k]][ii]) > tmp1[jj].first) {
                        ans.push_back({tmp1[jj].first, tmp1[jj].second *0.8* dis[k]});
                        ++jj;
                    } else {
                        ans.push_back(graph[root[k]][ii++]);
                    }
    ...
    fvlist = fopen((path + "/ppr/vlist.bin").c_str(), "w");
