...
__global__ void calcuatePPR(const ll *csr_v, const Vertex *csr_e, const Vertex *csr_w, const Vertex *weight_sum,
                            bool *f1, Vertex *act, Vertex *act_num, ValueType *pi, ValueType *oldval, ValueType *newval,
                            int *iteration_id,int *maxiter, int *record,
                            int vert_num,int label_num, ValueType alpha
);
...
int main(int argc, char **argv) {
    ...
    thrust::device_vector<Vertex> d_csr_w(edge_num,0);
    thrust::device_vector<Vertex> d_weight_sum(vert_num,0);

    thrust::device_vector<bool> d_f1(vert_num, false);
    thrust::device_vector<Vertex> d_act(vert_num, 0);
    thrust::device_vector<Vertex> d_f(1, vert_num);

    thrust::device_vector<ValueType> d_pi(vert_num*label_num,0.0);
    thrust::device_vector<ValueType> d_oldval(vert_num*label_num,0);
    thrust::device_vector<ValueType> d_newval(vert_num*label_num,0);
    ...
    thrust::copy(graph.weight_sum.begin(), graph.weight_sum.end(), d_weight_sum.begin());

    thrust::copy(graph.pi.begin(), graph.pi.end(), d_pi.begin());
    thrust::copy(graph.labels.begin(), graph.labels.end(), d_oldval.begin());
    thrust::copy(graph.new_labels.begin(), graph.new_labels.end(), d_newval.begin());
...
__global__ void calcuatePPR(const ll *csr_v, const Vertex *csr_e, const Vertex *csr_w, const Vertex *weight_sum,
    ...
    volatile __shared__ int commr[THREADS_PER_BLOCK/THREADS_PER_WARP];//每个warp对应一个值
    ...
    volatile __shared__ int commr2[THREADS_PER_BLOCK];
        ...
        while (schedule_offset < total_active_verts_num) {
                ...
                while (__syncthreads_or(push_st < push_ed)) {
                    if (push_st < push_ed) {
                        ...
                        for(int k=0;k<label_num;k++){
                            msg=w*oldval[commr[0]+k];
                            atomicAdd(newval + v * label_num + k, msg);
                            f1[v]=true;
                        }
            ...
            while (progress < total) {
                ...
                while (scatter<(progress+THREADS_PER_BLOCK) && (row_start<row_end)) {
                    ...
                    commr2[scatter-progress] = u* label_num;
                    scatter++;
                    row_start++;
                }
                __syncthreads();
                ...
                if (thread_id < cur_batch_count) {
                    ...
                    w = csr_w[comm2[thread_id]];
                    for(int k=0;k<label_num;k++){
                        msg=w*oldval[commr2[thread_id]+k];
                        atomicAdd(newval + v * label_num + k, msg);
                        f1[v]=true;
                    }
                }
            ...
            if (vid < vert_num) {
                if (f1[vid]) {
                    ...
                    for(int i=0;i<label_num;i++){
                        sum+=newval[vid*label_num+i];
                    }

                    if (sum/weight_sum[vid] > 0.02) {//执行边界检测标准，符合条件将标志位设>置为1
                    //边界检测的方式需要修改
                        for(int i=0;i<label_num;i++){
                            delta=alpha*newval[vid*label_num+i]/weight_sum[vid];
                            newval[vid*label_num+i]=0.0;
                            oldval[vid*label_num+i]=delta;
                            pi[vid*label_num+i]+=delta;
                        }
                        thread_cnt = 1;
                    }
                }
