...
int main(int argc, char **argv) {
    ...
    ValueType *wal;
    cudaMalloc(&wal, sizeof(ValueType)*2000);
    ...
    ValueType rmax =1.0f/(graph.csr_v.size());
    int cnt = 0;
...
__global__ void calcuatePPR(const int *csr_v, const int *csr_e, const ValueType *csr_w,
    ...
    if (global_id == 0) { //每一个块中线程为0的id，source=1,第一个顶点的residual值初始化为1，
        ...
        *active_verts_num = 1;
        ...
        pagerank[source] += 0.8 * residual[source];
        g_mutex1 = 0;
        g_mutex2 = 0;
        g_mutex4 = 0;
        g_mutex5 = 0;
...
/*    if(global_id<*active_verts_num){
    ...
    while (*active_verts_num > 0&&l_iteration_id<2000) {
        ...
        *active_verts_num = 0;
        ...
        if (thread_id == 0) {
            ...
            if(l_iteration_id<2000)	iteration_act_num[l_iteration_id]= *active_verts_num;
