...
class Graph {
  public:
    ...
    Graph(string _data_folder) {
        ...
        this->csr_o = read_binary2vector(data_folder + "/csr_olist.bin");
        this->csr_r = read_binary2vector(data_folder + "/csr_rlist.bin");
        

        
        this->info = read_binary2vector(data_folder + "/info.bin");
        this->max_step = csr_o.size()-1;
	   ...
	   this->vaddr = vert_num + rule_num;
        ...
        cout<<"The step is: "<<max_step<<endl;
    ...
    ~Graph() {
        ...
        vector<Vertex>().swap(this->csr_o);
        vector<Vertex>().swap(this->csr_r);
