...
__global__ void init(const int *csr_v, const int *csr_e, ValueType *pagerank,
            ...
            int source, int *iteration_id, int max_step,int *iteration_num,int *iteration_act_num,
            int *iter);

__global__ void calcuate(const int *csr_v, const int *csr_e, ValueType *pagerank,
            ValueType *residual, ValueType *messages, int *active_vert,
            int *active_verts_num, bool *isactive, int *degree, int *csr_o, int *csr_r,const int vert_num,
            const int rule_num, const ValueType alpha, const ValueType rmax,
            int source, int *iteration_id, int max_step,int *iteration_num,int *iteration_act_num,
...
__global__ void barrier(const int *csr_v, const int *csr_e, ValueType *pagerank,
            ValueType *residual, ValueType *messages, int *active_vert,
            int *active_verts_num, bool *isactive, int *degree, int *csr_o, int *csr_r,const int vert_num,
            const int rule_num, const ValueType alpha, const ValueType rmax,
            int source, int *iteration_id, int max_step,int *iteration_num,int *iteration_act_num,
...
int main(int argc, char **argv) {
   ...
   // int *iter;
   // cudaMalloc(&iter, sizeof(int) * (graph.max_step));
    ...
    while (1) {
        ...
        cnt++;
	   	...
	   	init<<<MAX_BLOCKS_NUM, THREADS_PER_BLOCK>>>(
            ...
            graph.max_step,device_memory.iteration_num,device_memory.iteration_act_num, device_memory.iter);
		int iter=0;		
		while(iter++<100){
			...
			calcuate<<<MAX_BLOCKS_NUM, THREADS_PER_BLOCK>>>(
            		device_memory.csr_v, device_memory.csr_e,
           		device_memory.pagerankStream, device_memory.residualStream,
            		device_memory.messagesStream, device_memory.active_vert,
            		device_memory.active_verts_numStream,device_memory.isactive,device_memory.degree, 
            		device_memory.csr_o, device_memory.csr_r,
            		graph.vert_num,graph.rule_num, alpha, rmax, source, iteration_id,
            		graph.max_step,device_memory.iteration_num,device_memory.iteration_act_num,
            		device_memory.iter);

			barrier<<<MAX_BLOCKS_NUM, THREADS_PER_BLOCK>>>(
            		device_memory.csr_v, device_memory.csr_e,
            		device_memory.pagerankStream, device_memory.residualStream,
            		device_memory.messagesStream, device_memory.active_vert,
            		device_memory.active_verts_numStream,device_memory.isactive,device_memory.degree, 
			...
			CUDA_ERROR(cudaMemcpy(act, device_memory.active_verts_numStream, sizeof(int), cudaMemcpyDeviceToHost));
			...
			float runtime_iter = 0; //milliseconds
			cudaEventElapsedTime(&runtime_iter, start_iter, stop_iter);
			cout << runtime_iter/1000.0 << endl;
        ...
        while (local_iter < max_step ) {
            ...
            __syncthreads();
             __threadfence();
                ...
                if (thread_id == 0) {
                    ...
                    while (g_mutex5 <gridDim.x * (local_iter)) {}
            ...
            total_rule = csr_o[local_iter]-csr_o[local_iter-1];
                ...
                if (thread_id == 0) {
                    ...
                    while (g_mutex4 <gridDim.x * (local_iter)) {}
...
__global__ void barrier(const int *csr_v, const int *csr_e, ValueType *pagerank,
        ...
        volatile __shared__ size_t output_cta_offset;
        size_t thread_id = threadIdx.x;
        size_t schedule_1 = blockDim.x * blockIdx.x;
        size_t vid = 0;
	   typedef cub::BlockScan<int, THREADS_PER_BLOCK> BlockScan;
        __shared__ typename BlockScan::TempStorage block_temp_storage;
