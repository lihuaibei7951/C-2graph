...
const Vertex origin_edge = graph.origin_edge;
const Vertex rule_num = graph.origin_edge;
const Vertex edge_num = graph.edge_num;
const Vertex max_step = graph.max_step;
const Vertex vaddr = graph.vaddr;

const vector<Vertex> csr_v = graph.csr_v;
const vector<Vertex> csr_e = graph.csr_e;
const vector<Vertex> degree = graph.degree;
const vector<Vertex> order = graph.order;


//ValueType *degree = new ValueType[graph.get_number_vertices_new()];
int cnt=0;
const ValueType alpha = 0.2f;
const ValueType rmax = 0.1f*(1.0f/origin_edge);
...
void initQueue(BufferQueue *buffer, int MaxSize) {
	...
	cudaMallocHost(&reserve, sizeof(ValueType)*vaddr);
	cudaMallocHost(&residue, sizeof(ValueType)*vaddr);
...
void forward_push(ValueType* reserve, ValueType* residue, Vertex *global_ft_1, Vertex global_ft1_cnt) {
	...
	while(active_vert.size()>0&&iter<100) {
		...
		//规则遍历
	    Vertex local_iter = 0;
         for(local_iter=0;local_iter < max_step + 1;local_iter++){
       
              for(auto it = active_rule[local_iter].begin(); it != active_rule[local_iter].end(); ++it){
