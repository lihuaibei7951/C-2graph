...
__global__ void
calcuatePPR(const int *csr_v, const int *csr_e, ValueType *pagerank,
            ValueType *residual, ValueType *messages, int *active_vert,
            int *active_verts_num, bool *isactive, int *degree, int *order, const int vert_num,
            const int rule_num, const ValueType alpha, const ValueType rmax,
            int source, int *iteration_id, int max_step,int *iter);
...
int main(int argc, char **argv) {
    ...
    int *iteration_id;
    cudaMalloc(&iteration_id, sizeof(int) * 137);


    int *iter;
    cudaMalloc(&iter, sizeof(int) * (graph.max_step + 1));
	...
	while (1) {
		...
		calcuatePPR<ValueType><<<MAX_BLOCKS_NUM, THREADS_PER_BLOCK, 0, stream1>>>(
            ...
            device_memory.messagesStream1, device_memory.active_vert1,
            device_memory.active_verts_numStream1,
            device_memory.isactive1,device_memory.degree, device_memory.order, graph.vert_num,
            graph.rule_num, alpha, rmax, source++, iteration_id,
            graph.max_step,iter);
            ...
            device_memory.messagesStream2, device_memory.active_vert2,
            device_memory.active_verts_numStream2,
            device_memory.isactive2,device_memory.degree, device_memory.order, graph.vert_num,
            graph.rule_num, alpha, rmax, source++, iteration_id,
            graph.max_step,iter);
		...
		CUDA_ERROR(cudaMemcpyAsync(bufferqueue->front->reserve, device_memory.pagerankStream1,
				graph.vaddr*sizeof(ValueType), cudaMemcpyDeviceToHost, stream1));
		...
		CUDA_ERROR(cudaMemcpyAsync(bufferqueue->front->reserve, device_memory.pagerankStream2,
				graph.vaddr*sizeof(ValueType), cudaMemcpyDeviceToHost, stream2));
		...
		bufferqueue->front = bufferqueue->front->next;
		bufferqueue->length++;
		//cudaDeviceSynchronize();
		if (cnt%10 == 0) {//100
			cudaDeviceSynchronize();
...
calcuatePPR(const int *csr_v, const int *csr_e, ValueType *pagerank,
    ...
    schedule_offset_init = blockDim.x * blockIdx.x;

    __syncthreads(); // 确保所有线程块都看到 schedule_offset_init 的重置

    while (schedule_offset_init < rule_num * (max_step + 1)) {
        vid = schedule_offset_init + thread_id;

        if (vid < rule_num * (max_step + 1)) {
            isactive[vid + vert_num] = false;
        }
                ...
                while (
                    ...
                    while (__syncthreads_or(push_st < push_ed)) {
                        if (push_st < push_ed) {
                            ...
                            msg = ((1 - alpha) * commr[0]) /
                                  commd[0]; // outdeg of the selected s, not "u"

                            atomicAdd(messages + v, msg);
                            
                            isactive[v + rule_num * order[v]] =true;
...
///        
        ...
        while (local_iter < max_step + 1) {
            ...
            if (thread_id == 0) {
                ...
                while (g_mutex5 <gridDim.x *(iteration_id[l_iteration_id - 1] * (max_step + 1) +local_iter)) {}
            }
            ...
            __syncthreads();
                ...
                size_t vid = 0,vvid =0,vvvid =0;

                while (__syncthreads_or(schedule_2 < rule_num)) {
                    // 这个函数返回的是一个布尔值，表示所有线程块中至少有一个线程满足条件。
                    vid = schedule_2 + thread_id;
                    
                    vvid = vid + vert_num + rule_num * (local_iter - 1);
                    vvvid = vid + vert_num;
                    int thread_cnt = 0;
                    if (vid < rule_num) {
                        if (isactive[vvid]) {
                            //printf("Hello from the vvid:%d \n",vvid);
                            //printf("Hello from the vvvid:%d \n",vvvid);
                            residual[vid + vert_num] =messages[vid + vert_num];
                            // 用残差值来传递消息
                            messages[vid + vert_num] =0; 
                            // 消息值置0，保证下一次迭代没问题
                            isactive[vvid] = false;
                            thread_cnt = 1;
                        }
                    ...
                    __syncthreads();
                    if (thread_cnt > 0) {
                        active_vert[output_cta_offset + scatter] = vvvid;
                ...
                if (thread_id == 0) {
                    ...
                    while (g_mutex4 <gridDim.x * (iteration_id[l_iteration_id - 1] *(max_step + 1) + local_iter)) {}
            ...
            // 下一步是消息传递，不需要边界检测
                ...
                while (schedule_3 < rule_num) {
                    ...
                    if (idxx < iter[local_iter-1]) {
                        ...
                        // printf("tttttttt%d\n",idxx);
                        //printf("total_active_verts_num:%d\n",total_active_verts_num);
                        u = active_vert[idxx];
                        //printf("uuuu%d\n",u);
                        ru = residual[u];
                        residual[u] = 0;
                    ...
                    // while(1)
                    while (__syncthreads_or((e_end - e_start) >=
                                            THREADS_PER_BLOCK)) {
                        if ((e_end - e_start) >= THREADS_PER_BLOCK) {
                            comm[0][0] =
                                thread_id; // I (thread_id) want to process the
                        ...
                        while (__syncthreads_or(push_st < push_ed)) {
                            ...
                            push_st +=
                                THREADS_PER_BLOCK; // until all outgoing edges
                    ...
                    while (__any_sync(FULL_MASK,
                        ...
                        if ((e_end - e_start) >= THREADS_PER_WARP) {
                            comm[warp_id][0] =
                                lane_id; // threads in the "warp_id"-th warp try
                                         // to vote
                        }
                        if (comm[warp_id][0] == lane_id) {
                            comm[warp_id][1] =
                                e_start; // vertex owned by the "lane_id"-th
                        ...
                        size_t push_st = comm[warp_id][1] +
                                         lane_id; // process the "push_st"-th
                                                  // outgoing edge at first.
                        size_t push_ed = comm[warp_id][2];
