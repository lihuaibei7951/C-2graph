...
int main(int argc, char **argv) {
    ...
    int source = 12345678; // 101569
    ValueType alpha = 0.85f;
    ValueType rmax =0.0001; // 这一步条件变了,应该是原始边数目
            ...
            while (schedule_1 < total_active_verts_num) {
                ...
                while ( __syncthreads_or((e_end - e_start) >= THREADS_PER_BLOCK)) {
                    ...
                    while (__syncthreads_or(push_st < push_ed)) {
                        if (push_st < push_ed) {
                            ...
                            msg = (commr[0]) / commd[0]; // outdeg of the selected s, not "u"
                ...
                while (__any_sync(FULL_MASK,(e_end - e_start) >= THREADS_PER_WARP)) {
                    ...
                    while (__any_sync(FULL_MASK, push_st < push_ed)) {
                        if (push_st < push_ed) {
                            v = csr_e[push_st];
                            msg = ( commr[warp_id]) /(commd[warp_id]);
                            atomicAdd(messages + v, msg);
                            isactive[v] =true;
                        }
                ...
                while (progress < total) {
                    ...
                    if (thread_id < cur_batch_count) {
                        ...
                        msg = (commr2[thread_id]) / commd2[thread_id];
                        atomicAdd(messages + v, msg);
                        isactive[v] =true;
                    }
                    __syncthreads();
...
__threadfence();
    ...
    while (schedule_offset_rinit < max_step + 1) {
        ...
        while (__syncthreads_or(schedule_1 < vert_num)) {
            ...
            int thread_cnt = 0;
            if (vid < vert_num) {
				messages[vid] =messages[vid]*alpha + (1.0-alpha)/vert_num;
				if (isactive[vid]) {
				     if(messages[vid]-pagerank[vid] > rmax*degree[vid]){
						thread_cnt = 1;
