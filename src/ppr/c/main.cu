...
calcuatePPR(const int *csr_v, const int *csr_e, ValueType *pagerank,
            ...
            int source, int *iteration_id, int max_step,int *iter,int *iteration_num,int *iteration_act_num);
...
int *iteration_num = new int[1];
    ...
    ValueType rmax =0.1f * (1.0f / graph.origin_edge); // 这一步条件变了,应该是原始边数目
    ...
    while (1) {
        ...
        calcuatePPR<ValueType><<<MAX_BLOCKS_NUM, THREADS_PER_BLOCK>>>(
            ...
            device_memory.active_verts_numStream,
            device_memory.isactive,device_memory.degree, device_memory.order, graph.vert_num,
            graph.rule_num, alpha, rmax, source++, iteration_id,
            graph.max_step,iter,device_memory.iteration_num,device_memory.iteration_act_num);
 			if (cnt == 1) {
...
calcuatePPR(const int *csr_v, const int *csr_e, ValueType *pagerank,
    ...
    while (*active_verts_num > 0&&l_iteration_id<1000) { //*active_verts_num > 0       l_iteration_id<1
