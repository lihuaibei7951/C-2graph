...
__global__ void calcuatePPR(const int *row_ptr, const int *col_ind, ValueType *pagerank, 
		ValueType *residual, ValueType *messages, int *active_verts, int *active_verts_num,
...
int main(int argc, char **argv) {
	...
	//forward_push(graph.csr_v,graph.csr_e,vert_num,0);
	//forward_push2(graph.csr_v,graph.csr_e,vert_num,226005);
	cout << "\n==================== PPR with FORWARD PUSH starts ====================" << endl;

	int *iteration_id;
	cudaMalloc(&iteration_id, sizeof(int)*50000);

	
	
	int *iteration_num = new int[1];
	int *iteration_act_num = new int[50000];
	...
	ValueType rmax =0.01f/(graph.edge_num);
	int cnt = 0;
	...
	while(1){
		...
		if(graph.csr_v[source]==graph.csr_v[source+1]){
			++source;
			continue;
		}
		...
		calcuatePPR<ValueType><<<MAX_BLOCKS_NUM, THREADS_PER_BLOCK>>>(
				device_memory.csr_v, device_memory.csr_e, device_memory.pagerankStream,
	...
	CUDA_ERROR(cudaMemcpy(iteration_act_num, device_memory.iteration_act_num, sizeof(int)*50000, cudaMemcpyDeviceToHost));
...
////同步点	
	...
	while (*active_verts_num > 0&&l_iteration_id<200) {
...
////同步点			
		...
		while (schedule_offset < total_active_verts_num) {
			...
			if (idx < total_active_verts_num) {
				...
				//补全0出度点的发送方向为源顶点
					...
					row_start = row_ptr[source];
					row_end = row_ptr[source + 1];
		...
		*active_verts_num = 0;
		...
		while (__syncthreads_or(schedule_offset_barrir < vert_num)) {
			...
			if (vid < vert_num) {
				//if (isactive[vid]) {
					...
					messages[vid] = 0;
					isactive[vid] = false;
					if ((residual[vid]/(row_ptr[vid+1] - row_ptr[vid])) >= rmax) {//执行边界检测标准，符合条件将标志位设>置为1
		...
		if (thread_id == 0) {
			...
			if(l_iteration_id<50000)	iteration_act_num[l_iteration_id]= *active_verts_num;
...
void forward_push(vector<Vertex> csr_v,vector<Vertex> csr_e,int vert_num, int source) {

	//clock_t start = clock();
	ValueType rmax=0.1f/3128710;
	ValueType alpha=0.2;
	...
	ValueType* reserve = new ValueType[vert_num];
	ValueType* residue = new ValueType[vert_num];

	Vertex iteration_id = 1;
	set<Vertex> active_vert;
	active_vert.insert(source);
	reserve[source]=0.2;
	residue[source]=1.0;
...
void forward_push2(vector<Vertex> csr_v,vector<Vertex> csr_e,int vert_num, int source) {
	...
	active_vert.insert(source);
	reserve[source]=0.2;
	residue[source]=0.8/(csr_v[source+1]-csr_v[source]);
	set<Vertex> active_vertx;	
	Vertex iter = 0;
	while(active_vert.size()>0&&iter<100) {
...
//顶点遍历
		...
		for(auto it = active_vert.begin(); it != active_vert.end(); ++it){
         ...
         for(auto it = active_vertx.begin(); it != active_vertx.end(); ++it){
         	     v = *it;
			if(messages[v]/(csr_v[v+1]-csr_v[v]) > rmax) {
				reserve[v] += (messages[v] * alpha);
				residue[v]=(1-alpha)*messages[v]/(csr_v[v+1]-csr_v[v]);
				messages[v] = 0.0;
				active_vert.insert(v);
			}
         }
