...
__global__ void init(const int *row_ptr, const int *col_ind, ValueType *pagerank, 
...
template<typename ValueType>
__global__ void calcuate(const int *row_ptr, const int *col_ind, ValueType *pagerank, 
		ValueType *residual, ValueType *messages, int *active_verts, int *active_verts_num,
		bool *isactive, const int vert_num, const ValueType alpha, const ValueType rmax,
		int source, int* iteration_id,int *iteration_num,int *iteration_act_num); 

template<typename ValueType>
__global__ void barrier(const int *row_ptr, const int *col_ind, ValueType *pagerank, 
		ValueType *residual, ValueType *messages, int *active_verts, int *active_verts_num,
		bool *isactive, const int vert_num, const ValueType alpha, const ValueType rmax,
		int source, int* iteration_id,int *iteration_num,int *iteration_act_num); 
// Dump results
...
int main(int argc, char **argv) {
	...
	ValueType rmax =0.1f/(graph.edge_num);
	int cnt = 0;
	while(1){
		...
		init<ValueType><<<MAX_BLOCKS_NUM, THREADS_PER_BLOCK>>>(
		...
		while(iter++<100){
			...
			calcuate<ValueType><<<MAX_BLOCKS_NUM, THREADS_PER_BLOCK>>>(
				device_memory.csr_v, device_memory.csr_e, device_memory.pagerankStream,
				device_memory.residualStream, device_memory.messagesStream, device_memory.active_vert,
				device_memory.active_verts_numStream, device_memory.isactive, vert_num, alpha,
				rmax, source, iteration_id,device_memory.iteration_num,device_memory.iteration_act_num);
