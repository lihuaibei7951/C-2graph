...
class DeviceMemory {
  public:
    ...
    DeviceMemory(int _v_num, int _r_num, int _e_num)
        : vert_num(_v_num), rule_num(_r_num), edge_num(_e_num) {
    ...
    void CudaMemcpyGraph(Graph &graph) {
        ...
        ValueType *result = vec_to_arr(graph.degree);
        CUDA_ERROR(cudaMemcpy(degree, result, sizeof(ValueType) * vaddr,
                              cudaMemcpyHostToDevice));
        delete[] result;
  ...
  private:
    ...
    void CudaMallocData() {
        ...
        CUDA_ERROR(cudaMalloc(&degree, sizeof(ValueType) * vaddr));
        CUDA_ERROR(cudaMalloc(&csr_v, sizeof(Vertex) * (vaddr + 1 )));
        CUDA_ERROR(cudaMalloc(&active_vert, sizeof(Vertex) * vaddr));
        ...
        CUDA_ERROR(cudaMalloc(&isactive, sizeof(bool) * vaddr));

                		CUDA_ERROR(cudaMalloc(&iteration_num, sizeof(int)));
		CUDA_ERROR(cudaMalloc(&iteration_act_num, sizeof(int)*50000));
    }

    // using array to replace vector
    
    template <typename VType>
    VType *vec_to_arr(vector<VType> vec) {

        int vec_size = vec.size();
        VType *ptr = new VType[vec_size];
