...
class Graph {
  public:
    ...
    Graph(string _data_folder) {
        ...
        vector<int> de =read_binary2vector(data_folder + "/degree.bin");
	   ...
	   this->degree.resize(vaddr);
	   
        this->origin_edge = 0;
	   for(Vertex i = 0;i<de.size();i++){
	   		this->origin_edge += de[i];
	   		this->degree[i]=(float)de[i];
	   		
	   }
	   for(int i=0;i<rule_num;i++){
	   		this->degree[i+vert_num]=0.8;
	   }
    ...
    ~Graph() {
        ...
        vector<ValueType>().swap(this->degree);
