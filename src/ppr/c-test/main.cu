...
calcuatePPR(const int *csr_v, const int *csr_e, ValueType *pagerank,
            ...
            int *active_verts_num, bool *isactive, ValueType *degree, const int vert_num,
...
int main(int argc, char **argv) {
    ...
    ValueType rmax =0;
       // 0.1f * (1.0f / graph.origin_edge); // 这一步条件变了,应该是原始边数目
    ...
    while (1) {
        ...
        calcuatePPR<ValueType><<<MAX_BLOCKS_NUM, THREADS_PER_BLOCK>>>(
            ...
            device_memory.isactive,device_memory.degree, graph.vert_num,
            graph.rule_num, alpha, rmax, source++, iteration_id,device_memory.iteration_num,device_memory.iteration_act_num);
 		if (cnt == 1) {
        	cudaDeviceSynchronize();
            break;
        }
...
__global__ void calcuatePPR(const int *csr_v, const int *csr_e, ValueType *pagerank,
    ...
    while (*active_verts_num > 0&&l_iteration_id<50000) { //*active_verts_num > 0       l_iteration_id<1
        ...
        *active_verts_num = 0;
        ...
        while (__syncthreads_or(schedule_1 < vert_num+ rule_num)) {
...
//				}
				...
				if((residual[vid]/(degree[vid])) > rmax&&degree[vid] != 0.8) {//执行边界检测标准，符合条件将标志位设>置为1
					pagerank[vid] += alpha * residual[vid];
					thread_cnt = 1;
				}
