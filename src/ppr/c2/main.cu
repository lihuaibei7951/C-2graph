...
__global__ void calcuatePPR(const int *csr_v, const int *csr_e, ValueType *pagerank,
            ...
            int *active_verts_num, bool *isactive, int *degree, const int vert_num,
     ...
     while (1) {
		...
		//cout << "source = " << source << "start ------------" <<cnt<< endl;
         ...
         calcuatePPR<<<MAX_BLOCKS_NUM, THREADS_PER_BLOCK>>>(
            ...
            device_memory.iteration_num,device_memory.iteration_act_num);
            if (cnt == 1) {
        	      cudaDeviceSynchronize();
...
__global__ void calcuatePPR(const int *csr_v, const int *csr_e, ValueType *pagerank,
    ...
    while (*active_verts_num > 0&&l_iteration_id<200) { //*active_verts_num > 0       l_iteration_id<1
        ...
        ValueType ru, msg;
            ...
            while (schedule_1 < total_active_verts_num) {
                ...
                if (idx < total_active_verts_num) {
                    u = active_vert[idx];
                    if(u<vert_num) ru = (1-alpha)*residual[u]/degree[u];
                    else ru = residual[u];
                    residual[u] = 0;
                ...
                while (__any_sync(FULL_MASK,(e_end - e_start) >= THREADS_PER_WARP)) {
                    ...
                    while (__any_sync(FULL_MASK, push_st < push_ed)) {
                        if (push_st < push_ed) {
                            v = csr_e[push_st];
                            atomicAdd(messages + v, commr[warp_id]);
                            isactive[v] =true;
                        }
                ...
                while (progress < total) {
                    ...
                    if (thread_id < cur_batch_count) {
                        ...
                        atomicAdd(messages + v, commr2[thread_id]);
                        isactive[v] =true;
                    }
                    __syncthreads();
	  ...
	  __threadfence();
       if (thread_id == 0) {
            atomicAdd((int *)&g_mutex3, 1);
            while (g_mutex3 < gridDim.x * iteration_id[l_iteration_id]) {
            }
        }
        __syncthreads();
        ...
        while (__syncthreads_or(schedule_1 < vaddr)) {
