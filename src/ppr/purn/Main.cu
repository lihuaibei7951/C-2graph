...
__global__ void calcuatePPR(const int *csr_v, const int *csr_e, const ValueType *csr_w,
                            ...
                            int source, int* iteration_id,int *iteration_num,int *iteration_act_num,const ValueType *ww);
...
int main(int argc, char **argv) {
    ...
    ValueType rmax =0.001f/(graph.csr_v.size());
    int cnt = 0;
    ...
    while(1){
        ...
        CUDA_ERROR(cudaMemcpy(wal+init_active_num, &graph.csr_w[graph.csr_v[source]],
                              sizeof(float)*(graph.csr_v[source+1]-graph.csr_v[source]), cudaMemcpyHostToDevice));
                              cudaDeviceSynchronize();
        init_active_num += graph.csr_v[source+1]-graph.csr_v[source];
        cudaDeviceSynchronize();

        CUDA_ERROR(cudaMemcpy(device_memory.active_verts_numStream,&init_active_num,
        ...
        calcuatePPR<ValueType><<<MAX_BLOCKS_NUM, THREADS_PER_BLOCK>>>(
                ...
                rmax, source, iteration_id,device_memory.iteration_num,device_memory.iteration_act_num,wal);
...
//        pagerank[active_verts[global_id]]+=(1-alpha)*alpha*ww[global_id];
//        residual[active_verts[global_id]]+=(1-alpha)*ww[global_id];
        atomicAdd(&pagerank[active_verts[global_id]], (1 - alpha) * alpha * ww[global_id]);
        atomicAdd(&residual[active_verts[global_id]], (1 - alpha) * ww[global_id]);
    }
