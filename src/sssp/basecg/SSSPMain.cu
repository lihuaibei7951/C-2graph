...
//kernel_ update
//template<typename ValueType>
__global__ void init(const Vertex *csr_v,const Vertex *csr_e, const ValueType *csr_w, ValueType *dis, 
		Vertex *active_vert, Vertex *active_vert_num, bool *isactive,  
		const Vertex vert_num, Vertex source,Vertex *iteration_id, Vertex *iteration_num ,Vertex *iteration_act_num);  
int main(int argc, char **argv) {
    ...
    bool flag = (bool)(std::stoi(argv[2]));
    // 获取，csr_v ,csr_e ,v_r,degree,order;
    
    Graph graph(dir,flag);
	...
	int root = 121;
	int source;
	if(!flag) source = graph.csr_idx[root];
	else source = root;
	
	cudaStream_t stream1;
	cudaStreamCreate(&stream1);
	
	init<<<MAX_BLOCKS_NUM, THREADS_PER_BLOCK,0,stream1>>>(
	...
	while (1) {
		...
		//ofstream outfile("/home/lhb/cucode/SSSP/SSSP_GPU/out/result/a11.txt");
		cnt++;
		...
		while (schedule_offset < total_avtive_num) {
			...
			while (__syncthreads_or((row_end-row_start)>=THREADS_PER_BLOCK)) {
				...
				while (__syncthreads_or(push_st<push_ed)) {
					if (push_st < push_ed) {
						...
						weight = dis[commr[0]]+csr_w[push_st];
						if( dis[v]>weight){
			...
			while (__any_sync(FULL_MASK, (row_end-row_start)>=THREADS_PER_WARP)) {
				...
				while (__any_sync(FULL_MASK, push_st<push_ed)) {
					if (push_st < push_ed) {
						v = csr_e[push_st];
						weight = dis[commr[warp_id]] + csr_w[push_st];
						if( dis[v]>weight){
							atomicMin(dis+v, weight);
							isactive[v] = true;
						}
			...
			while (progress < total) {
				...
				if (thread_id < cur_batch_count) {
					...
					weight = dis[commr2[thread_id]]+csr_w[comm2[thread_id]];
					if( dis[v]>weight){
							atomicMin(dis+v, weight);
							isactive[v] = true;
						}
