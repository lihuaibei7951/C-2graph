...
public:
    ...
    void Graphinit(string _data_folder) {
        ...
        this->csr_v = read_binary2vector(x,data_folder + "/sssp/vlist.bin");
        this->csr_e = read_binary2vector(x,data_folder + "/sssp/elist.bin");
        
        this->csr_w = read_binary2vector(y,data_folder + "/sssp/wlist.bin");
        ...
        for(int i=0;i<vert_num;i++){
            ...
            maxw=max(maxw,csr_w[j]);
            csr_w[j]=(csr_w[j]%100)+1;
                indegree[csr_e[j]]+=1;
                if(csr_v[i+1]-csr_v[i]>20){
                     if( (j-csr_v[i])%2 ){
                     	csr_w[j]=100000001;
                     }
                }
