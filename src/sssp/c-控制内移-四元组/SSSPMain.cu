...
__global__ void CalcuSSSP(const Vertex *csr_v, const Vertex *csr_e, const ValueType *csr_w, ValueType *dis,
                          ...
                          Vertex *active_vert1,Vertex *active_vert2, ValueType *active_w1,ValueType *active_w2,
                          Vertex *active_vert_num1,Vertex *active_vert_num2, bool *isactive,
...
int main(int argc, char **argv) {
    ...
    int *iteration_id;
    iteration_id = NULL;
    cudaMalloc(&iteration_id, sizeof(int) * 200);

    int *iteration_num = new int[1];
    int *ac1 = new int[1];
    int *ac2 = new int[1];
    int *iteration_act_num = new int[200];
    int source = 1;
    ...
    while (1) {
		...
		CUDA_ERROR(cudaMemset(device_memory.iteration_num, 0, sizeof(int)));
            CalcuSSSP<<<MAX_BLOCKS_NUM, THREADS_PER_BLOCK,0,stream1>>>(
                    ...
                    device_memory.active_vert1,device_memory.active_vert2, 
                    device_memory.active_w1,device_memory.active_w2, 
                    device_memory.active_vert_num1,device_memory.active_vert_num2, device_memory.isactive,
...
__global__ void CalcuSSSP(const Vertex *csr_v, const Vertex *csr_e, const ValueType *csr_w, ValueType *dis,
    ...
    volatile __shared__ ValueType commw[THREADS_PER_BLOCK / THREADS_PER_WARP];//每个warp对应个值
    ...
    volatile __shared__ ValueType commw2[THREADS_PER_BLOCK];//每个warp对应3个值

    while ((*active_vert_num1 >0||*active_vert_num2 >0)&&l_iteration_id<200) {
        l_iteration_id += 1;
        __syncthreads();
        ...
        if(l_iteration_id%2==1){
            ...
            size_t idx=0;
                ...
                ValueType ww=0.0,du=0.0, weight=0.0; //dis value of u
            ...
            while (schedule_offset < total_avtive_num1) {
                ...
                if (idx < total_avtive_num1) {
                    ...
                    ww = active_w1[idx];//指向当时记录的距离/根节点距离
                    du = dis[root];//最新的距离
                    if(ww>du){
                    row_start = 0;
                    row_end = 0;
			 ...
			 while (__syncthreads_or((row_end - row_start) >= THREADS_PER_BLOCK)) {
                ...
                while (__any_sync(FULL_MASK, (row_end - row_start) >= THREADS_PER_WARP)) {
                    ...
                    while (__any_sync(FULL_MASK, push_st < push_ed)) {
                        if (push_st < push_ed) {
                //        if(commw[warp_id]==commr[warp_id]){
                ...
                //    if(commr2[thread_id]==commw2[thread_id]){
                        ...
                        if(v < vert_num){
                            ...
                            active_vert2[cur+2]=testx;
                            active_w2[cur/3] =commw2[thread_id];
            ...
            *active_vert_num1 = 0;
            ...
            if (threadIdx.x == 0) {
                ...
                if(l_iteration_id<200)	iteration_act_num[l_iteration_id]= *active_vert_num2/3;
            }
            __syncthreads();
            ...
            while (schedule_offset < total_avtive_num1) {
                ...
                if (idx < total_avtive_num1) {
                    ...
                    ww = active_w2[idx];//指向当时记录的距离/根节点距离
                    du = dis[root];//最新的距离
                    if(ww>du){
                    row_start = 0;
                    row_end = 0;
                ...
                while (__any_sync(FULL_MASK, (row_end - row_start) >= THREADS_PER_WARP)) {
                    ...
                    while (__any_sync(FULL_MASK, push_st < push_ed)) {
                        if (push_st < push_ed) {
                      //  if(commr[warp_id] ==commw[warp_id]){
                            v = csr_e[push_st];
                            int testx = commi[warp_id][1] + csr_idx[push_st];
                            if(v<vert_num){
                ...
                while (progress < total) {
                  ...
                  //  if(commr2[thread_id] ==commw2[thread_id]){
                        ...
                        if(v < vert_num){
                            ...
                            active_vert1[cur+2]=testx;
                            active_w1[cur/3] = commw2[thread_id];
            ...
            *active_vert_num2 = 0;
            ...
            if (threadIdx.x == 0) {
                iteration_num[0] = l_iteration_id;
                if(l_iteration_id<200)	iteration_act_num[l_iteration_id]= *active_vert_num1/3;
            }
            __syncthreads();
