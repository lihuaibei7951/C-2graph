...
class DeviceMemory {
  ...
  private:
    ...
    void CudaMallocData() {
        ...
        CUDA_ERROR(cudaMalloc(&iteration_num1, sizeof(int)));
	   CUDA_ERROR(cudaMalloc(&iteration_act_num1, sizeof(int)*200));
	   ...
	   CUDA_ERROR(cudaMalloc(&distance2, sizeof(ValueType) * vert_num ));  
        ...
        CUDA_ERROR(cudaMalloc(&iteration_num2, sizeof(int)));
	   CUDA_ERROR(cudaMalloc(&iteration_act_num2, sizeof(int)*200));
    }
    ...
    void CudaFreeData() {
        ...
        if (iteration_num1) CUDA_ERROR(cudaFree(iteration_num1));
	   if (iteration_act_num1) CUDA_ERROR(cudaFree(iteration_act_num1));
	   ...
	   if (active_vert2)
        ...
        if (iteration_num2) CUDA_ERROR(cudaFree(iteration_num2));
	   if (iteration_act_num2) CUDA_ERROR(cudaFree(iteration_act_num2));
    }
};
