...
int main(int argc, char **argv) {
    ...
    cudaFree(0);
    // 获取命令行参数
    std::string dd = argv[1];

    
	DeviceMemory device_memory(vert_num, edge_num);
	...
	initQueue(bufferqueue, 100);//1000
	...
	while (1) {
		...
		CalcuSSSP<<<MAX_BLOCKS_NUM, THREADS_PER_BLOCK,0,stream1>>>(
				...
				device_memory.active_vert1,device_memory.active_vert_num1,device_memory.isactive1,  
				vert_num, source++,iteration_id,device_memory.iteration_num1,device_memory.iteration_act_num1);
			
				
		cnt++;

		CUDA_ERROR(cudaMemcpyAsync(bufferqueue->front->distance, device_memory.distance1,
					vert_num *sizeof(ValueType), cudaMemcpyDeviceToHost, stream1 ));//1
	...
	CalcuSSSP<<<MAX_BLOCKS_NUM, THREADS_PER_BLOCK,0,stream2>>>(
				...
				vert_num, source++,iteration_id,device_memory.iteration_num2,device_memory.iteration_act_num2);
		...
		cudaDeviceSynchronize();
			bufferqueue->flag = -1;
			cout << "flag 已设置成 -1  终止条件以满足" << endl;
			//cout << "buffer length = " << bufferqueue->length << endl;
			break;
		}
...
__global__ void CalcuSSSP(const Vertex *csr_v,const Vertex *csr_e, ValueType *csr_w, ValueType *dis, 
	...
	while (l_iteration_id < 50&&*active_vert_num > 0) {
	//while (*active_vert_num > 0) {
	int total_act = *active_vert_num;
		l_iteration_id += 1;
  				__syncthreads();
	if (threadIdx.x == 0) {
		...
		while (schedule_offset < total_act) {
		...
		*active_vert_num = 0;
		...
		if (threadIdx.x == 0) {
			...
			if(l_iteration_id<200)	iteration_act_num[l_iteration_id]= *active_vert_num;
		}
		__syncthreads();
