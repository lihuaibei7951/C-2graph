...
__global__ void InitSSSP(const Vertex *csr_v, const Vertex *csr_e, const ValueType *csr_w, ValueType *dis,
...
template<typename ValueType>
__global__ void Barrier(const Vertex *csr_v, const Vertex *csr_e, const ValueType *csr_w, ValueType *dis,
                          const Vertex *csr_ov, const Vertex *csr_idx,
                          Vertex *active_vert1,Vertex *active_vert2, Vertex *active_vert_num1,Vertex *active_vert_num2, bool *isactive,
                          const Vertex vert_num, Vertex source, Vertex *iteration_id, Vertex *iteration_num,
                          Vertex *iteration_act_num);
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        InitSSSP<<<MAX_BLOCKS_NUM, THREADS_PER_BLOCK>>>(
        ...
        *ac2=1;
        while(iter++<100&&(*ac2!=0)){
            ...
            Barrier<<<MAX_BLOCKS_NUM, THREADS_PER_BLOCK>>>(
                    ...
                    device_memory.active_vert1,device_memory.active_vert2, device_memory.active_vert_num1,device_memory.active_vert_num2, device_memory.isactive,
                    vert_num, source, iteration_id, device_memory.iteration_num, device_memory.iteration_act_num);
            //CUDA_ERROR(cudaMemcpy(ac1, device_memory.active_vert_num2,sizeof(int), cudaMemcpyDeviceToHost));


            CalcuSSSP<<<MAX_BLOCKS_NUM, THREADS_PER_BLOCK>>>(
                    device_memory.csr_v, device_memory.csr_e, device_memory.csr_w, device_memory.distance,
                    device_memory.csr_ov, device_memory.csr_idx,
                    device_memory.active_vert2,device_memory.active_vert1, device_memory.active_vert_num2,device_memory.active_vert_num1, device_memory.isactive,
                    vert_num, source, iteration_id, device_memory.iteration_num, device_memory.iteration_act_num);
            ...
            CUDA_ERROR(cudaMemcpy(ac2, device_memory.active_vert_num1,sizeof(int), cudaMemcpyDeviceToHost));
        }
        source++;
        cnt++;
...
__global__ void CalcuSSSP(const Vertex *csr_v, const Vertex *csr_e, const ValueType *csr_w, ValueType *dis,
    ...
    int total_avtive_num = *active_vert_num1/3;

    size_t thread_id = threadIdx.x;//当前块内的线程id
